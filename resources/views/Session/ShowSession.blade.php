            ...
            <div class="row">
                <div class="col-md-8 col-md-offset-2">
                    <ol class="breadcrumb">
                        ...
                        <li class="active">Gerer les sessions</li>
                    </ol>
                </div>
            </div>
        ...
        <div class="row">
            <div class="col-md-8 col-md-offset-2">
                <div class="panel panel-info ">
                    <div class="panel-heading"><strong>Liste des sessions:</strong> <span class="badge">{{ \App\Session::count() }} session(s) de formation</span> <a style="float: right" href="{{ route('AddSession') }}" title="Ajouter une nouvelle session" class="glyphicon glyphicon-plus-sign"></a> </div>
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <table class="table table-striped table-responsive" >
                                    ...
                                    @foreach($AllSession as $CF)
                                    <tr id="{{$CF->id}}">
                                        <td> {{$CF->CODE}}<d>
                                        <td>{{$CF->LIBELLE}}</td>
                                        <td>{{$CF->START}}</td>
                                        <td>{{$CF->END}}</td>
                                        <td> {!! ($CF->ACTIVE ==1) ? '<a href="'.route('DisableSession',$CF->id).'"><span  class="glyphicon glyphicon-eye-open" aria-hidden="true"></span></a>' : '<a href="'.route('EnableSession',$CF->id).'"><span  class="glyphicon glyphicon-eye-close" aria-hidden="true"></span></a>' !!}</td>
                                        ...
                                        <div class="modal fade" id="ModalEdit{{$CF->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel">
                                            <div class="modal-dialog" role="document">
                                                <form action="UpdateSession" method="post" name="CAT{{$CF->id}} " id="CAT{{$CF->id}}">
                                                    {{csrf_field()}}
                                                    ...
                                                    <div class="modal-body">
                                                        ...
                                                        <div class="form-group">
                                                            <label for="message-text" class="control-label">Debut:</label>
                                                            <input required style="text-transform:uppercase" type="date" class="form-control" name="START" id="START" value="{{$CF->START}}">
                                                        </div>

                                                        <div class="form-group">
                                                            <label for="message-text" class="control-label">Fin:</label>
                                                            <input required style="text-transform:uppercase" type="date" class="form-control" name="END" id="END" value="{{$CF->END}}">
                                                        </div>
                                        ...
                                        <div class="modal fade" id="ModalDel{{$CF->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel">
                                            <div class="modal-dialog" role="document">
                                                <form action="DelSession" method="post" name="CATDel{{$CF->id}} " id="CAT{{$CF->id}}">
                                                    {{csrf_field()}}
                                                    <input type="hidden" name="id" id="id" value="{{$CF->id}}">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                                                            <h4 class="modal-title" id="exampleModalLabel">Supprimer session N°{{$CF->id}}</h4>
                                                        </div>
                                                        <div class="modal-body">
